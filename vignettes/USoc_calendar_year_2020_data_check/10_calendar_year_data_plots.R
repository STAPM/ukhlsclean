...
ggplot(quali_sex) +
  aes(x = d_sex, y = prop ) +
  geom_col(aes(fill = forcats::fct_rev(d_hiqual))) +
...
ggplot(econstat_sex) +
  aes(x = d_sex, y = prop ) +
  geom_col(aes(fill = forcats::fct_rev(l_econ_stat_3cat))) +
...
earn <- earn[l_econ_stat_7cat == "employed" & year == data_year & d_age %in% 16:64,
              c("id","pop_factor","wave_no","d_sex","l_grss_earnings_last_empl","l_grss_earnings_usual_empl")]
...
cpi_index <- as.numeric(ukhlsclean::cpi[month == 4 & year == data_year,"cpi_value"])
...
mean <- weighted.mean(earn$l_grss_earnings_usual, w = earn$pop_factor, na.rm = TRUE)
...
ggplot(earn) +
  ...
  labs(y = "", x = "Gross Annual Earnings (000s)",
       caption = "Earnings inflated to April 2020 | Note that earnings are top-coded to £100,000 per annum") +
  theme_custom() +
