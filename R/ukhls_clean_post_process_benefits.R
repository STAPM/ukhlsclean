...
ukhls_post_clean_benefit <- function(data = NULL) {
  ...
  data[b_jbseek_allowance != "receipt" & b_UniCred != "receipt", unemployment_benefits := "non_receipt"]
  data[b_jbseek_allowance != "receipt" & b_UniCred == "receipt" & l_econ_stat_3cat != "unemployed", unemployment_benefits := "non_receipt"]
  data[is.na(b_jbseek_allowance) & is.na(b_UniCred), unemployment_benefits := NA]
