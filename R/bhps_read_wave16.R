#' Read British Household Panel Survey Wave 16
#'
#' Reads and does basic cleaning on the BHPS 16th wave.
#'
#' MISSING VALUES
#'
#' \itemize{
...
#' }
#' @export
bhps_read_wave16 <- function(
  root = c("C:/"),
  path = "Users/User/Documents/Datasets/UKHLS/tab/"
) {

  print("Reading BHPS Wave 16")
  data <- data.table::fread(
    paste0(root[1], path, "bhps_w16/bp_indresp.tab"),
  ...
  demographic_vars <- colnames(data[ , c(33,1235,32,1406,91)])
  econ_stat_vars   <- colnames(data[,c(38)])
  weight_vars      <- colnames(data[,c(1317,1315)])
  ...
  data.table::setnames(data,
                       c("pid","bp_hidp","bp_pno",
                         ## demographic
                         "bp_sex","bp_age_dv","bp_doby","bp_gor_dv","bp_racel_bh",
                         ## economic stauts
                         "bp_jbstat",
                         ## weight
                         "bp_xrwght","bp_lrwght"),
