#' Read British Household Panel Survey Wave 6
#'
#' Reads and does basic cleaning on the BHPS 6th wave.
#'
#' MISSING VALUES
#'
#' \itemize{
...
#' }
#' @export
bhps_read_wave6 <- function(
  root = c("C:/"),
  path = "Users/User/Documents/Datasets/UKHLS/tab/"
) {

  print("Reading BHPS Wave 6")
  data <- data.table::fread(
    paste0(root[1], path, "bhps_w6/bf_indresp.tab"),
  ...
  demographic_vars <- colnames(data[ , c(30,740,29,885,78)])
  econ_stat_vars   <- colnames(data[,c(31)])
  weight_vars      <- colnames(data[,c(820,818)])
  ...
  data.table::setnames(data,
                       c("pid","bf_hidp","bf_pno",
                         ## demographic
                         "bf_sex","bf_age_dv","bf_doby","bf_gor_dv","bf_race",
                         ## economic stauts
                         "bf_jbstat",
                         ## weight
                         "bf_xrwght","bf_lrwght"),
