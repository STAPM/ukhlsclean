#' Read British Household Panel Survey Wave 13
#'
#' Reads and does basic cleaning on the BHPS 13th wave.
#'
#' MISSING VALUES
#'
#' \itemize{
...
#' }
#' @export
bhps_read_wave13 <- function(
  root = c("C:/"),
  path = "Users/User/Documents/Datasets/UKHLS/tab/"
) {

  print("Reading BHPS Wave 13")
  data <- data.table::fread(
    paste0(root[1], path, "bhps_w13/bm_indresp.tab"),
  ...
  demographic_vars <- colnames(data[ , c(33,1183,32,1385,39)])
  econ_stat_vars   <- colnames(data[,c(38)])
  weight_vars      <- colnames(data[,c(1299,1297)])
  ...
  data.table::setnames(data,
                       c("pid","bm_hidp","bm_pno",
                         ## demographic
                         "bm_sex","bm_age_dv","bm_doby","bm_gor_dv","bm_racel_bh",
                         ## economic stauts
                         "bm_jbstat",
                         ## weight
                         "bm_xrwght","bm_lrwght"),
