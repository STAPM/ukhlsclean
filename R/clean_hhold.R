...
clean_hhold <- function(data = NULL) {
  ...
  data$hh_type_6cat <- as.factor(data$hh_type_6cat)
  ...
  data$hh_age_yngchl <- factor(data$hh_age_yngchl,
                               levels = c("no children","age 0-2","age 3-4","age 5-11","age 12-15"))
  ...
  data$hh_hometenure <- factor(data$hh_hometenure,
                               levels = c("owner occupier","private renter","social renter"))
