...
clean_smoke <- function(data = NULL) {
  ...
  data[dataset == "UKHLS" & wave_no < 23 & age >= StartAge & age <= QuitAge, current_smoker := TRUE] # backfill current smoker status if in prior waves was within the smoking age range
  data[dataset == "UKHLS" & wave_no < 23 & is.na(current_smoker), current_smoker := FALSE]
  ...
  data[dataset == "UKHLS" &
         ...
         age >= StartAge &
         age < QuitAge,
       ncigs := ifelse(!is.nan(MeanCigs),MeanCigs,MeanCurrCigs ) ]

  ## identify current smokers ------------------
  data[(wave_no == 23 & smnow == 1) |
       (wave_no > 23 & smoker == 1) |
       (is.na(smoker) & ncigs > 0), current_smoker := TRUE, by=pidp]
  data[dataset == "UKHLS" & is.na(current_smoker), current_smoker := FALSE]

  ## identify former smokers ------------------
  data[wave == 23 & smever == 1 & current_smoker == FALSE, former_smoker := TRUE]
