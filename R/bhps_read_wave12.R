#' Read British Household Panel Survey Wave 12
#'
#' Reads and does basic cleaning on the BHPS 12th wave.
#'
#' MISSING VALUES
#'
#' \itemize{
...
#' }
#' @export
bhps_read_wave12 <- function(
  root = c("C:/"),
  path = "Users/User/Documents/Datasets/UKHLS/tab/"
) {

  print("Reading BHPS Wave 12")
  data <- data.table::fread(
    paste0(root[1], path, "bhps_w12/bl_indresp.tab"),
  ...
  demographic_vars <- colnames(data[ , c(33,1121,32,1318,120)])
  econ_stat_vars   <- colnames(data[,c(38)])
  weight_vars      <- colnames(data[,c(1235,1233)])
  ...
  data.table::setnames(data,
                       c("pid","bl_hidp","bl_pno",
                         ## demographic
                         "bl_sex","bl_age_dv","bl_doby","bl_gor_dv","bl_race",
                         ## economic stauts
                         "bl_jbstat",
                         ## weight
                         "bl_xrwght","bl_lrwght"),
