#' Read British Household Panel Survey Wave 3
#'
#' Reads and does basic cleaning on the BHPS 3rd wave.
#'
#' MISSING VALUES
#'
#' \itemize{
...
#' }
#' @export
bhps_read_wave3 <- function(
  root = c("C:/"),
  path = "Users/User/Documents/Datasets/UKHLS/tab/"
) {

  print("Reading BHPS Wave 3")
  data <- data.table::fread(
    paste0(root[1], path, "bhps_w3/bc_indresp.tab"),
  ...
  demographic_vars <- colnames(data[ , c(30,685,29,850,78)])
  econ_stat_vars   <- colnames(data[,c(31)])
  weight_vars      <- colnames(data[,c(776,774)])
  ...
  data.table::setnames(data,
                       c("pid","bc_hidp","bc_pno",
                         ## demographic
                         "bc_sex","bc_age_dv","bc_doby","bc_gor_dv","bc_race",
                         ## economic stauts
                         "bc_jbstat",
                         ## weight
                         "bc_xrwght","bc_lrwght"),
