#' Read British Household Panel Survey Wave 17
#'
#' Reads and does basic cleaning on the BHPS 17th wave.
#'
#' MISSING VALUES
#'
#' \itemize{
...
#' }
#' @export
bhps_read_wave17 <- function(
  root = c("C:/"),
  path = "Users/User/Documents/Datasets/UKHLS/tab/"
) {

  print("Reading BHPS Wave 17")
  data <- data.table::fread(
    paste0(root[1], path, "bhps_w17/bq_indresp.tab"),
  ...
  demographic_vars <- colnames(data[ , c(33,1115,32,1288,91)])
  econ_stat_vars   <- colnames(data[,c(38)])
  weight_vars      <- colnames(data[,c(1199,1197)])
  ...
  data.table::setnames(data,
                       c("pid","bq_hidp","bq_pno",
                         ## demographic
                         "bq_sex","bq_age_dv","bq_doby","bq_gor_dv","bq_racel_bh",
                         ## economic stauts
                         "bq_jbstat",
                         ## weight
                         "bq_xrwght","bq_lrwght"),
